...
function M.new(params)
  if params.sock then return params end
  params.call_semantics = params.call_semantics or 'new'

  -- Create a TCP cosocket.
  local err
  request[params],err = tcp()
  ...
  if params.socket_timeout then
    request[params]:settimeout()
  end
...
function M.disconnect(self)
  if not request[self] then
    return nil, 'No socket created.'
  end
  return request[self]:close()
end
...
function M.set_keepalive(self)
  ...
  local ok, err = request[self]:setkeepalive()
...
local function tnt_request(self, header, body)
  local sock = request[self]
...
local function handshake(self)
  ...
  -- it is ok.
  if request[self]:getreusedtimes() >0 then return true end
  ...
  local greeting, greeting_err = request[self]:receive(iproto.greeting_size)
...
M.handshake = handshake -- this is needed to establish a new connection maintained by the external interface
...
function M.connect(self, host, port)
  ...
  local ok, err = request[self]:connect(host or self.host, port or self.port, self.socket_options)
  if not ok then
    return ok, err
  end
...
function M.ping(self)
  ...
  local response, err = tnt_request(self, { [packet_keys.type] = command_keys.ping })
  if err then
    return nil, err
  elseif response and response.code ~= response_keys.ok then
    return nil, response and response.error or 'Internal error.'
...
local function get_index_id(self, space, index)
  ...
  local spaceid, err = self._spaces[index] or get_space_id(self, space)
  -- If there was an error return it.
...
function M.select(self, space, index, key, opts)
  ...
  local indexid,err = self._indexes[index] or get_index_id(self, spaceid, index)
  if not indexid then
    return nil, err
  end
